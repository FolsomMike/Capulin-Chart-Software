...
hdwVs.setSimDataType(pConfigFile.readInt(section, "Simulation Data Type", 0));

//create the arrays to hold data points and flag/decoration info
if (sizeOfDataBuffer > 100000) sizeOfDataBuffer = 100000;

dataBuffer1 = new int[sizeOfDataBuffer]; flagBuffer = new int[sizeOfDataBuffer];
...
//draw the key text, outline it with a black rectangle, and store the dimensions
//of the text so mouse clicks on the key can be detected
...
traceGlobals.bufOffset = 0; //left edge of screen starts at buffer position 0
...
//this variable is used to determine if a new segment has started and thus has
//data which can be saved
...
//increment to the next buffer position - it is assumed that new data is ready
//there, if at the end of the buffer, wrap to the beginning.
...
//if plotting span style, draw line between high and low peak in vertical line
pG2.setColor(traceColor);
...
//if there is a flag set for this data point then draw it - threshold indexes
//are shifted by two as 0 = no flag and 1 = user flag
