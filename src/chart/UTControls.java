...
//If one grows but not the other, the smaller one will be stretched to fit the
//window and will look bad.  Make both grids have the same row count so they
//will be the same size.  If the row count is less than four, make it four to
//keep the rows from stretching.

// if current channel is null, then only the chart stuff will be displayed so
// make the grid large enough to hold the thresholds, igoring gates

if (currentChannel != null)
    gridYCount = (currentChannel.numberOfGates > chart.getNumberOfThresholds())
...
//setup the chart controls tab before checking if a valid channel is available
//so the user can still hide or display the chart
setupChartTab();
...
//there may be more rows than thresholds because the number of rows may be set
//higher to prevent them from stretching to fill empty space
//for those extra rows, fill in dummy labels
...
//avoid runtime errors but this seems pointless as the cast MUST work for the
//program to work so it will crash regardless if the cast is bad
...
if ("Delete DAC Gate".equals(e.getActionCommand())){
    ...
    //recalculate the time locations from the pixel locations for all DAC gates
...
//try casting the source component to a spinner - if valid, then get the name
//if the component is a threshold control, then update chart settings only
...
try{
    ...
    System.err.println("Component type which called stateChanged is unknown.");
    }
...
//using the references to each adjust control stored when the panel was setup,
//scan through each threshold storing the values in the threshold object
...
//using the references to each adjust control stored when the panel was setup,
//scan through each gate storing the values in the gate object
...
if (displayMode == 3)
    ((OscopeCanvas)oscopeCanvas).setVertOffset(oscopeCanvas.getHeight() / 2 );
else
...
//clear the selected flag for the currently selected gate
//will do nothing if no gate is selected
currentChannel.setSelectedDACGate(currentChannel.getSelectedDACGate(), false);
...
if (e.getButton() == MouseEvent.BUTTON1){
    //force recalc of all gate time locations and flag for sending to remotes
